...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw std::runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"bitstock-address\" monthy-payment\n"
            ...
            "5. \"bitstock-address\":   (string, required) Bitstock address to send payments to\n"
...
UniValue submitbudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 7)
        throw std::runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"bitstock-address\" monthly-payment \"fee-tx\"\n"
