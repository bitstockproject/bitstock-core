...
#if defined(NDEBUG)
#error "Bitstock cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Bitstock
...
{
    RenameThread("bitstock-scriptch");
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Bitstock Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().BitstockBadBlockTime()) && (block.nBits == (uint32_t) Params().BitstockBadBlocknBits())) {
            // accept Bitstock block minted with incorrect proof of work threshold
            return true;
        }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Bitstock: We use certain sporks during IBD, so check to see if they are
