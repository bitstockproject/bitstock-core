Bitstock Core version *3.1.1* is now available from:  <https://github.com/bitstock-altc/bitstock/releases>
...
Bitstock Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.
...
### Fix spending for v1 zBSOCK created before block 1050020

The transition to v2 zBSOCK and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zBSOCK to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zBSOCK v1 will now be able to convert that into BSOCK and then zBSOCK v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zBSOCK stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by Bitstock stakers.
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zBSOCK code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zBSOCK elements if needed. (presstab)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zBSOCK created before block 1050020. (presstab)
