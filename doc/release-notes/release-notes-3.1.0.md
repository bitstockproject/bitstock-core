Bitstock Core version *3.1.0* is now available from:  <https://github.com/bitstock-altc/bitstock/releases>
...
zBSOCK Staking is here! zBSOCK staking will be activated on the morning of the 8th of May 2018. With the release of zBSOCK staking, there are effectively 2 versions of zBSOCK, zBSOCK minted on the 3.0.6 Bitstock wallet or lower, and zBSOCK minted on Bitstock wallet version or higher. New features in this release will require the use of zBSOCK v2, zBSOCK minted on this wallet release 3.1.0 or later. If you currently hold zBSOCK v1 and wish to take advantage of zBSOCK staking and deterministic zBSOCK, you will need to spend the zBSOCK v1 to yourself and remint zBSOCK v2.
Note: To find your zBSOCK version, click the privacy tab, then the zBSOCK Control button then expand the arrows next to the desired denomination.


### Deterministic zBSOCK Seed Keys

zBSOCK is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zBSOCK outside of the wallet that the zBSOCK had been minted on. zBSOCK can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zBSOCK minting

zBSOCK minting now only requires 1 further mint (down from 2) to mature. zBSOCK mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zBSOCK Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zBSOCK denomination has been spent or not.



BSOCK/zBSOCK Staking and Masternode Rewards
--------------

### BSOCK, zBSOCK and Masternode Payment Schedule

To encourage the use of zBSOCK and increase the Bitstock zerocoin anonymity set, the Bitstock payment schedule has been changed to the following:

If a user staking zBSOCK wins the reward for their block, the following zBSOCK reward will be: 
- 3 zBSOCK (3 x 1 denominations) rewarded to the staker, 2 BSOCK rewarded to the masternode owner and 1 BSOCK available for the budget. This is a total block reward of 6 BSOCK, up from 5.

If a user staking BSOCK wins the reward, the following amounts will be rewarded: 
- 2 BSOCK to the BSOCK staker, 3 BSOCK to the Masternode owner and 1 BSOCK available for the budget. This is a total block reward of 6 BSOCK, up from 5.


### Return change to sender when minting zBSOCK

Previously, zBSOCK minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most BSOCK and return the change from a zBSOCK mint to that address.
...
-	Enable and disable the auto zBSOCK minting feature. This is enabled by default and the enablezeromint=0 setting in the bitstock.conf file will overwrite the GUI option.
-	The percentage of autominted zBSOCK can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular BSOCK.
...
* If zBSOCK backup, auto generated name is `wallet-autozpivbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the Bitstock support group, which often times sees users say "I didn't spend that zBSOCK". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the Bitstock masternodes, the monthly budget available to be utilised has been increased to 42,000 BSOCK / month. This BSOCK only has the opportunity to be raised once per month (paid to winning proposals) with any unused BSOCK not created by the blockchain.

### Budget Finalization Fee

The Bitstock finalization fee for successful proposals has now been reduced, this fee is now 5 BSOCK down from 50 BSOCK. The total fee outlay for a successful proposal is now a total of 55 BSOCK.
...
-	Enforced v1 zBSOCK spends to require a security level of 100
-	Updates to zBSOCK spends to avoid segfaults
...
Several critical security flaws in the zerocoin protocol and Bitstock's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zBSOCK Control dialog in the QT wallet, a user is able to see zBSOCK marked as version 1 or 2.

zPoS (zBSOCK staking)
--------------

Once a zBSOCK has over 200 confirmations it becomes available to stake. Staking zBSOCK will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zBSOCK. So for example if a 1,000 zBSOCK denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zBSOCKs.
...
Zerocoins, or zBSOCK, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zBSOCK that can be recalculated at any time using the seed. Deterministic zBSOCK allows for users to backup all of their future zBSOCK by simply recording their seed and keeping it in a safe place (similar to backing up a private key for BSOCK). The zBSOCK seed needs to remain in the wallet in order to spend the zBSOCK after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zBSOCK data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.
...
### Core Features
 - #582 `cb1632520` [Core] zBSOCK v2: zPoS dzBSOCK ezBSOCK external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zBSOCK balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zBSOCK status output (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zBSOCK spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zBSOCK. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zBSOCK. (presstab)
