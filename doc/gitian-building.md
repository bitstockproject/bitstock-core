...
*Setup instructions for a gitian build of Bitstock Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Bitstock
...
to the Bitstock GitHub release page.
...
- [Building Bitstock Core](#building-bitstock-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/bitstock-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/bitstock-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/bitstock-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/bitstock-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/bitstock-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/bitstock-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://https://github.com/Bitstock-blockchain/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@bitstock.investments) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/bitstock-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/bitstock-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/bitstock-osx-*-build.assert
```
